...
export default function SendNewsletterForm() {
    const [send_state, formAction] = useFormState(sendNewsletter, initialState)
    const { pending } = useFormStatus();
    const statusmsg = send_state.status === "success" ? <SubmitAlert alert_type="success" header="Success" msg={send_state.message} show_time={4000} />
      : send_state.status === "fail" ? <SubmitAlert alert_type="error" header="Error" msg={send_state.message} show_time={4000} />
      : "";
